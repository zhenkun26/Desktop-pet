...
> 结论:**未发现 P0 级必现崩溃/数据丢失/安全入侵路径;7 项 P1 已全部修复;81 个测试全绿;核心服务范围行覆盖率 86.27%**。
...
npx tsc --noEmit
npm test                    # 81 个测试
npm run test:coverage       # 核心服务范围行覆盖率 86.27%
...
| `npm test` | **13 个测试文件 / 81 个测试全部通过** |
| 覆盖率(核心服务范围) | 语句 86.27% / 行 86.27% / 分支 84.87% / 函数 81.48% |
| 模块亮点 | sse 97.14% · personas 100% · secrets-store 88.88% · chat-db 84.37% · timer 88.7% |

新增测试覆盖:事件监听器隔离、SSE 解析(跨块/CRLF/畸形行)、DeepSeek 错误映射与流式组装、SQLite CRUD 与上下文过滤、密钥存取/损坏降级、配置损坏回退、计时器状态机与参数归一化、人设构建、角色注册表、聊天发送全链路(成功/中止/失败/缺 Key/隔离)、userData 目录迁移(幂等/跳过/复制)。
